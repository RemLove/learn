...
void merge(int arr[], int help[], int l,int m,int r)//m是左边的边界
{
	int a = l;
	int b = m + 1;
	int i= l;
...
void MergeSort1(int arr[],int help[],int l,int r)
{
	if (l == r) return;
	int m = (l + r) / 2;
	MergeSort1(arr, help, l, m);
	MergeSort1(arr, help, m + 1, r);
	merge(arr, help, l, m, r);
}
//4.2归并排序(非递归版)
int Min(int a, int b)
...
void MergeSort2(int arr[], int help[],int size)
{
	for (int step = 1; step < size; step <<= 1)
	{
		int l = 0;
...
//5.随机快排（荷兰国旗问题）
int first = 0;//是等于x的左边界，初始时在l位置，后面不断往后移
int last = 0;//是等于x的右边界，初始时在r位置，后面不断往前移
void partition(int arr[],int l,int r, int x)
{
	first = l;
	last = r;
...
void Quick_Sort(int arr[], int l,int r)
{
	...
	partition(arr, l, r, x);
	int leftEnd = first - 1;
	int rightStart = last + 1;
	Quick_Sort(arr, l, leftEnd);
	Quick_Sort(arr, rightStart, r);
}
//封装一下
...
void HeapSort2(int arr[], int size)//从底往顶建堆
{
